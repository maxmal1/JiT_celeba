...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        if num_cls_token > 0:
            ...
            cos, sin = torch.cat([cos_pad, cos_img], dim=0), torch.cat([sin_pad, sin_img], dim=0)  # [N_cls+N_img, D]
        else:
            cos, sin = freqs.cos().view(-1, freqs.shape[-1]), freqs.sin().view(-1, freqs.shape[-1])

        # buffers follow model.to(device); non-persistent so they stay out of checkpoints
